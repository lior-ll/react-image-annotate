...
export default (function (state, action) {
  ...
  var proprgateRegion = function proprgateRegion(regionId, proprgate, refRegion) {
    /* 
    proprgate actions:
    ...
    */
    ...
    var keyframeTimes = Object.keys(state.keyframes).map(function (a) {
      return parseInt(a);
    }).filter(function (a) {
      return !isNaN(a);
    }).filter(function (a) {
      return a > (state.currentVideoTime || 0);
    });
    
    keyframeTimes.forEach(function (keyframe) {
      //console.error(keyframe);
      var oldKeyframeRegions  = [...(getIn(state, ["keyframes", keyframe || 0, "regions"]))];
      var _getRegion = getRegion(regionId),
        _getRegion2 = _slicedToArray(_getRegion, 2),
        region = _getRegion2[0],
        regionIndex = _getRegion2[1];
      var idxv = oldKeyframeRegions.findIndex(item => region && (region.id == item.id));
      //console.error("oldKeyframeRegions", oldKeyframeRegions);
      //console.error("idxv", idxv);
      ...
      switch (proprgate) {
        case "proprgate-to-all":
          if (idxv < 0)
            oldKeyframeRegions.push(region);
         // else
         //   console.error("new box alreasy exist");
          break;
        case "proprgate-attributes":
          if (idxv > -1) 
          {
            oldKeyframeRegions[idxv] = setIn(oldKeyframeRegions[idxv], ["cls"], refRegion.cls);
            oldKeyframeRegions[idxv] = setIn(oldKeyframeRegions[idxv], ["comment"], refRegion.comment);
            oldKeyframeRegions[idxv] = setIn(oldKeyframeRegions[idxv], ["tags"], refRegion.tags);
            oldKeyframeRegions[idxv] = setIn(oldKeyframeRegions[idxv], ["color"], refRegion.color);
          }
          break;
        case "proprgate-delete":
          if (idxv > -1)
            oldKeyframeRegions.splice(idxv, 1);
          break;
        case "proprgate-to-simillar-only":
          if (idxv > -1)
            if (oldKeyframeRegions[idxv].x === refRegion.x && oldKeyframeRegions[idxv].y === refRegion.y && oldKeyframeRegions[idxv].w === refRegion.w && oldKeyframeRegions[idxv].h === refRegion.h)
              oldKeyframeRegions[idxv] = region;
          break;
        default:
          break;
      }
      //console.error("after prop", oldKeyframeRegions);
      state = setIn(saveToHistory(state, "Prpoprgate region to Keyframe"), ["keyframes", keyframe || 0], {
        regions: oldKeyframeRegions
      });
    });
    return state;
  };
  ...
  var modifyRegion = function modifyRegion(regionId, obj) {
    ...
    if (obj !== null) {
      state = setIn(state, [].concat(_toConsumableArray(pathToActiveImage), ["regions", regionIndex]), _objectSpread({}, region, obj));
      return state; //return proprgateRegion()
    } else {
      // delete region
    ...
    case "CHANGE_REGION":
      {
        ...
        state = setIn(state, [].concat(_toConsumableArray(pathToActiveImage), ["regions", regionIndex]), action.region);
    ...
    case "MOUSE_UP":
      {
        ...
        var original = state.mode.original;
        var regId = state.mode.regionId;
        var isNew = state.mode.isNew;
        switch (state.mode.mode) {
          case "RESIZE_BOX":
            {
              if (state.mode.isNew) {
                if (Math.abs(state.mode.original.x - _x2) < 0.002 || Math.abs(state.mode.original.y - _y2) < 0.002) {
                  state = setIn(modifyRegion(state.mode.regionId, null), ["mode"], null);
                }
              }
              
              if (state.mode.editLabelEditorAfter) {
                state = _objectSpread({}, modifyRegion(state.mode.regionId, {
                  editingLabels: true
                }), {
                  mode: null
                });
    ...
    case "DELETE_REGION":
      {
        ...
        state = setIn(state, [].concat(_toConsumableArray(pathToActiveImage), ["regions"]), (activeImage.regions || []).filter(function (r) {
